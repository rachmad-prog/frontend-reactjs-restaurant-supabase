...
function App() {
  ...
  const confirmOrderCompleted = async (invoiceNumber) => {
    const token = localStorage.getItem("token");
    if (!token || !invoiceNumber) return;
    try {
      ...
      if (data.token) {
        window.snap.pay(data.token, {
          onSuccess: () => {
            ...
            onSuccess();
            // confirmOrderCompleted(data.orderId).then(fetchOrderHistory);
            confirmOrderCompleted(data.invoiceNumber ?? data.orderId).then(
              fetchOrderHistory,
            );
            Swal.fire({
              toast: true,
